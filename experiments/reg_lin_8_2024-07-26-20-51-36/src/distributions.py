...
def sample_normal(params_dict):
    try:
        x = np.random.normal(params_dict['mean'], np.sqrt(params_dict['sigma_sq']), params_dict['size']).reshape(-1, 1)
    except:
...
def sample_y_squared_gamma_keep(x, params_dict):
    """
    Sample y as a linear function of x, with noise depending on gamma
    """
    alpha = params_dict['alpha']
